...
def get_model(dir):
    ...
    args.det_model_dir = os.path.join(dir, "det_infer")
    args.cls_model_dir = os.path.join(dir, "cls_infer")

    args.rec_model_dir = os.path.join(dir, "rec_infer_2")
    args.use_angle_cls = True
    return TextSystem(args)


def get_sml_model(dir):
    args = utility.parse_args()
    args.det_model_dir = os.path.join(dir, "det_infer")
    args.cls_model_dir = os.path.join(dir, "cls_infer")
    args.rec_model_dir = os.path.join(dir, "rec_infer_2")
    args.det_db_unclip_ratio = 4.0
    return TextSystem(args)
