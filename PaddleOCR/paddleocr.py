...
class PaddleOCR(predict_system.TextSystem):
    ...
    def ocr(self,
        ...
        def my_preprocess_image(_image):
            # 将图像转换为灰度
            gray = cv2.cvtColor(_image, cv2.COLOR_BGR2GRAY)
            # gaus = cv2.GaussianBlur(gray, (3, 3), 0)
            edges = cv2.Canny(gray, 50, 150, apertureSize=3)
            # 使用霍夫直线变换检测直线
            lines = cv2.HoughLinesP(edges, 0.3, np.pi / 180, threshold=50, minLineLength=30, maxLineGap=1)
            result = _image.copy()
            for line in lines:
                x1, y1, x2, y2 = line[0]
        ...
        def preprocess_image(_image):
            ...
            # _image = my_preprocess_image(_image)  # my code
            return _image
