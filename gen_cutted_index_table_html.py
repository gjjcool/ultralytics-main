...
from UltralyticsYOLO.toolkit.index_table_html_gen import IndexTableHTMLGen
...
yolo_det_index_table_elem = YOLO("UltralyticsYOLO/runs/server/index_elem_0/weights/best.pt")
...
def predict_core(data_dict, img, index, pageId):
    ...
    #
    # 检测并识别 明细表元素
    page = data_dict.get_page(index)
    page['targets'].append({
        'box': [[0.0, 0.0], [1.0, 1.0]],
        'class': TargetTypeTag.indexTable,
        'confidence': 1.0
    })
    DetAndRecIndexTableElem(img, data_dict.get_page(index), yolo_det_index_table_elem, small_area_text_rec)

    # 检测并识别 目录材料表元素
    # DetAndRecListTableElem(img, data_dict.get_page(index), yolo_det_list_table_elem, small_area_text_rec)
...
if __name__ == "__main__":
    ...
    input_dir = r'/root/drawing_rec/table0524/index/group_19/'
    html_output_dir = r'/root/drawing_rec/table_quota/index_15/html'
    table_img_output_dir = r'/root/drawing_rec/table_quota/index_15/images'

    # input_dir = args.input
    # html_output_dir = os.path.join(args.output, 'html')
    # table_img_output_dir = os.path.join(args.output, 'images')
    ...
    for img_path, file_name in fo.find_all_file_paths(input_dir):
        ...
        # 过滤无用字段
        filter_field(data_dict.data)
        IndexTableHTMLGen(rawImg, data_dict.get_page(0), html_output_dir=html_output_dir,
