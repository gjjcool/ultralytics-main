...
def predict_core(data_dict, img, index, pageId, raw_img):
    ...
    # 检测并识别 主表元素
    DetAndRecMainTableElem(img, data_dict.get_page(index), yolo_det_main_table_elem, small_area_text_rec, raw_img)
    #
    # # 检测并识别 明细表元素
...
if __name__ == "__main__":
    with open('main_table_val_conf.json', 'r', encoding='utf-8') as conf_file:
    ...
    for img_path, file_name in fo.find_all_file_paths(input_dir):
        ...
        MainTableHTMLGen(rawImg, data_dict.get_page(0), html_output_dir=html_output_dir,
