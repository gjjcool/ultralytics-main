...
def small_area_text_rec(img, test=False, isDet=True):
    def det_and_rec(image):
        ...
        if len(res) == 0:
            if test:
                cv2.imwrite(
                    r'D:\Resources\Projects\Python\ultralytics-main-2\val\cell3' + f"\\[LEN=0]{comm.get_time_str()}.png",
                    img)
            return ''
        ...
        if len(res) == 1:
            if test:
                cv2.imwrite(
                    r'D:\Resources\Projects\Python\ultralytics-main-2\val\cell3' + f"\\[{res[0][0]}]{comm.get_time_str()}.png",
                    img)
            return res[0][0]
        ...
        if test:
            cv2.imwrite(
                r'D:\Resources\Projects\Python\ultralytics-main-2\val\cell3' + f"\\[LENgt1][{sb}]{comm.get_time_str()}.png",
                img)
        return sb
    ...
    def rec(image):
        ...
        if test:
            cv2.imwrite(
                r'D:\Resources\Projects\Python\ultralytics-main-2\val\cell3' + f"\\[{res[0][0]}]{comm.get_time_str()}_rec.png",
                img)
        return res[0][0]
    ...
    if text.strip() == '':
        # TODO: 将图片水平方向拉长，再检测+识别（det_and_rec）一次，若结果不为空则返回，若为空则调用 rec 并返回
        return rec(img)
    return text
...
async def predict_bt(files, pageIdList, graphId, callbackUrl, bucketName):
    data_dict = None
    try:
        ...
        for index, file_contents in enumerate(files):
            if isinstance(file_contents, str):
                print(f'OUT DOWNLOADING FILE: [{bucketName}] [{file_contents}]')
        ...
        async with httpx.AsyncClient() as client:
            while callbackCnt < callbackPatience:
                ...
                try:
                    ...
                    response = await client.post(callbackUrl, json=data_dict.data, timeout=10 + 20 * callbackCnt)
                    print('------RESPONSE--------')
                ...
                await asyncio.sleep(random.randint(callbackCnt, 15))
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        for file in final_files:
            ...
            for img, rgb_img in zip(imgs, rgb_imgs):
                # # ---
                # cv2.imwrite(
                #     r'D:\Resources\Projects\Python\ultralytics-main-2\val\png' + f"\\[bin]{comm.get_time_str()}.png",
                #     img)
                # cv2.imwrite(
                #     r'D:\Resources\Projects\Python\ultralytics-main-2\val\png' + f"\\[raw]{comm.get_time_str()}.png",
                #     rgb_img)
                # # ---
                j += 1
                data_dict = ResultFileGen('TEST')
...
def main():
    ...
    uvicorn.run('zysd_server:app', host=conf['host'], port=conf['port'], reload=False)
